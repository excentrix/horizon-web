...
| | Horizon | VELO |
|---|---|---|
| Mark | the sun-over-horizon mark | **the same** sun mark |
| Wordmark | lowercase `horizon` | uppercase `VELO` |
| Lockup | mark + `horizon` | mark + `VELO` + `by excentrix` byline |
| Voice | the AI mentor that knows you | proof of work, not promises |
| Lives in | `(frontend)` route group | `(velo)` route group |

Both belong to **Excentrix · Bangalore** — which appears only in footer/legal, never in the lockup.
