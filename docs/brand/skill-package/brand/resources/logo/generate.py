...
rasters and favicons. Mirrors the on-site logos:
  ...
  - VELO lockup   : components/velo/VeloWordmark.tsx (shared sun mark + "VELO" + "by excentrix" byline)
...
H_MARK, H_TEXT, H_GAP = 280.0, 232.0, 100.0          # lockup: mark28 text23.2 gap10 (Horizon + VELO)
...
V_TAG, V_TAGGAP, V_TAGTRACK = H_TEXT * 0.5, 70.0, 0.20   # "by excentrix" byline: half VELO, baseline-aligned

def velo_lockup_svg(mark_hex, text_hex, tag_hex):
    # VELO reuses the Horizon sun mark; the "by excentrix" byline gives context.
    tr = trace(BRIC, "VELO")
    tg = trace(MONO, "BY EXCENTRIX", tracking_em=V_TAGTRACK)
    s = H_TEXT/tr.upm
    st = V_TAG/tg.upm
    tx = H_MARK + H_GAP - tr.xmin*s
    baseline = H_MARK/2 + (tr.ymax+tr.ymin)/2*s      # center ink to mark center
    vmaxx = tx + tr.xmax*s
    ttx = vmaxx + V_TAGGAP - tg.xmin*st
    tmaxx = ttx + tg.xmax*st
    top = min(0.0, baseline - tr.ymax*s)
    bot = max(H_MARK, baseline - tr.ymin*s)
    p = 12.0
    mark = (f'  <g transform="scale({H_MARK/100:.5f})">'
            f'<rect width="100" height="100" fill="{mark_hex}" mask="url(#b)"/></g>')
    text = "  " + text_svg_el(tr, text_hex, H_TEXT, tx, baseline)
    tag = "  " + text_svg_el(tg, tag_hex, V_TAG, ttx, baseline)   # baseline-aligned byline
    defs = f'  <defs>\n{MASK}\n  </defs>\n'
    return _svg(-p, top-p, tmaxx+2*p, (bot-top)+2*p, mark+"\n"+text+"\n"+tag, defs)
...
def velo_lockup_png(height, mark_hex, text_hex, tag_hex, path, ss=4):
    H = int(height*ss)
    markH = int(H*(H_MARK/max(H_MARK, H_TEXT)))  # mark is tallest element
    m = mark_image(markH, mark_hex)
    fs = int(H*(H_TEXT/H_MARK)); gap = int(H*(H_GAP/H_MARK))
    fnt = ImageFont.truetype(BRIC, fs)
    asc, desc = fnt.getmetrics()
    tw = int(fnt.getlength("VELO"))
    tagfs = int(fs*0.5); taggap = int(H*(V_TAGGAP/H_MARK)); track = tagfs*V_TAGTRACK
    tfnt = ImageFont.truetype(MONO, tagfs)
    ttw = int(sum(tfnt.getlength(c) for c in "BY EXCENTRIX") + track*len("BY EXCENTRIX"))
    W = markH + gap + tw + taggap + ttw + ss*8
    cv = Image.new("RGBA", (W, markH), (0, 0, 0, 0))
    cv.alpha_composite(m, (0, 0))
    base = markH//2 + (asc-desc)//2
    _draw_text(cv, (markH+gap, base), "VELO", fnt, rgba(text_hex))
    _draw_text(cv, (markH+gap+tw+taggap, base), "BY EXCENTRIX", tfnt, rgba(tag_hex), tracking_px=track)
    cv = cv.crop(cv.getbbox())
    cv = cv.resize((max(1, cv.width//ss), max(1, cv.height//ss)), Image.LANCZOS)
    cv.save(path)
...
def main():
    ...
    # ---- VELO lockup (shared sun mark + VELO wordmark + "by excentrix" byline)
    write(P("velo", "velo-lockup-color.svg"), velo_lockup_svg(HEX["energy"], HEX["ink"], HEX["charcoal"]))
    write(P("velo", "velo-lockup-ink.svg"),   velo_lockup_svg(HEX["ink"], HEX["ink"], HEX["charcoal"]))
    write(P("velo", "velo-lockup-cream.svg"), velo_lockup_svg(HEX["cream"], HEX["cream"], HEX["cream"]))
    write(P("velo", "velo-lockup-white.svg"), velo_lockup_svg(HEX["white"], HEX["white"], HEX["cream"]))
    for h in (64, 128, 256):
        velo_lockup_png(h, HEX["energy"], HEX["ink"], HEX["charcoal"], P("velo", f"velo-lockup-color-{h}.png"))
        velo_lockup_png(h, HEX["cream"], HEX["cream"], HEX["cream"], P("velo", f"velo-lockup-white-{h}.png"))
