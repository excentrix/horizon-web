...
### `velo/` — VELO wordmark lockup (`VELO` + `by excentrix`)
| File | Use |
|---|---|
| `velo-lockup-ink.svg` | **default** — ink `VELO` + charcoal tag (light backgrounds) |
| `velo-lockup-white.svg` / `-cream.svg` | reversed, for dark backgrounds |
| `velo-lockup-ink-{64,128,256}.png`, `-white-{…}.png` | rasters (height in px) |
